...
class Page:
	...
	def addTextCtrl(self, name, labelWidth, ht, validator):
		...
		st = wx.StaticText(self, wx.ID_ANY, self.labels[name], size=(labelWidth, -1), style=wx.ALIGN_RIGHT)
		lsz.Add(st)
...
class MechanicalPage(wx.Panel, Page):
	def __init__(self, parent, nb, idPg):
		...
		self.spmKeys = [ 'STEPS_PER_M_X', 'STEPS_PER_M_Y', 'STEPS_PER_M_Z', 'STEPS_PER_M_E' ]
						
		self.mfrKeys = [ 'MAXIMUM_FEEDRATE_X', 'MAXIMUM_FEEDRATE_Y', 'MAXIMUM_FEEDRATE_Z', 'MAXIMUM_FEEDRATE_E' ]
		
		self.msrKeys = [ 'SEARCH_FEEDRATE_X', 'SEARCH_FEEDRATE_Y', 'SEARCH_FEEDRATE_Z' ]
		
		self.eclKeys = [ 'ENDSTOP_CLEARANCE_X', 'ENDSTOP_CLEARANCE_Y', 'ENDSTOP_CLEARANCE_Z' ]
		
		self.minmaxKeys = [	'X_MIN', 'X_MAX', 'Y_MIN', 'Y_MAX', 'Z_MIN', 'Z_MAX' ]
		
		self.labels = { 'STEPS_PER_M_X': "X:", 'STEPS_PER_M_Y': "Y:", 'STEPS_PER_M_Z': "Z:", 'STEPS_PER_M_E' : "E:",
						'MAXIMUM_FEEDRATE_X': "X:", 'MAXIMUM_FEEDRATE_Y': "Y:", 'MAXIMUM_FEEDRATE_Z': "Z:", 'MAXIMUM_FEEDRATE_E': "E:",
						'SEARCH_FEEDRATE_X': "X:", 'SEARCH_FEEDRATE_Y': "Y:", 'SEARCH_FEEDRATE_Z': "Z:",
						'ENDSTOP_CLEARANCE_X': "X:", 'ENDSTOP_CLEARANCE_Y': "Y:", 'ENDSTOP_CLEARANCE_Z': "Z:",
						'X_MIN': "Min X:", 'X_MAX': "Max X:", 'Y_MIN': "Min Y:", 'Y_MAX': "Max Y:", 'Z_MIN': "Min Z:", 'Z_MAX': "Max Z:",
...
\\***************************************************************************/\n")
		...
		for k in self.spmKeys + self.mfrKeys + self.msrKeys + self.eclKeys + self.minmaxKeys:
...
class AccelerationPage(wx.Panel, Page):
	...
	def insertValues(self, cfgValues):
		...
		for tag in [ 'ACCELERATION_REPRAP', 'ACCELERATION_RAMPING', 'ACCELERATION_TEMPORAL' ]:
...
class PinoutsPage(wx.Panel, Page):
	def __init__(self, parent, nb, idPg):	
		...
		self.pinXKeys = [ ('X_STEP_PIN', 0), ('X_DIR_PIN', 0), ('X_MIN_PIN', 0), ('X_MAX_PIN', 0), ('X_ENABLE_PIN', 0),
				('X_INVERT_DIR', 1), ('X_INVERT_MIN', 1), ('X_INVERT_MAX', 1), ('X_INVERT_ENABLE', 1) ]
		self.pinYKeys = [ ('Y_STEP_PIN', 0), ('Y_DIR_PIN', 0), ('Y_MIN_PIN', 0), ('Y_MAX_PIN', 0), ('Y_ENABLE_PIN', 0),
				('Y_INVERT_DIR', 1), ('Y_INVERT_MIN', 1), ('Y_INVERT_MAX', 1), ('Y_INVERT_ENABLE', 1) ]
		self.pinZKeys = [ ('Z_STEP_PIN', 0), ('Z_DIR_PIN', 0), ('Z_MIN_PIN', 0), ('Z_MAX_PIN', 0), ('Z_ENABLE_PIN', 0),
				('Z_INVERT_DIR', 1), ('Z_INVERT_MIN', 1), ('Z_INVERT_MAX', 1), ('Z_INVERT_ENABLE', 1) ]
		self.pinEKeys = [ ('E_STEP_PIN', 0), ('E_DIR_PIN', 0), ('E_ENABLE_PIN', 0), ('E_INVERT_DIR', 1), ('E_INVERT_ENABLE', 1) ]
		self.pinMiscKeys = [ ('USE_INTERNAL_PULLUPS', 1), ('TX_ENABLE_PIN', 0), ('RX_ENABLE_PIN', 0),
						('PS_ON_PIN', 0), ('PS_MOSFET_PIN', 0), ('STEPPER_ENABLE_PIN', 0), ('STEPPER_INVERT_ENABLE', 1),						
						('SD_CARD_DETECT', 0), ('SD_WRITE_PROTECT', 0), ('DEBUG_LED_PIN', 0) ]
		
		self.labels = { 'INCLUDE_ARDUINO': "Include arduino.h header file", 'USE_INTERNAL_PULLUPS': "Use Internal Pullups",
					
						'TX_ENABLE_PIN': "Tx Enable Pin:", 'RX_ENABLE_PIN': "Rx Enable Pin:",
						
						'X_STEP_PIN': "Step Pin:", 'X_DIR_PIN': "Direction Pin:", 'X_MIN_PIN': "Minimum Pin:", 'X_MAX_PIN': "Maximum Pin:",
						'X_ENABLE_PIN': "Enable Pin:", 'X_INVERT_DIR': "Invert Direction", 'X_INVERT_MIN': "Invert Minimum",
						'X_INVERT_MAX': "Invert Maximum", 'X_INVERT_ENABLE': "Invert Enable",
						
						'Y_STEP_PIN': "Step Pin:", 'Y_DIR_PIN': "Direction Pin:", 'Y_MIN_PIN': "Minimum Pin:", 'Y_MAX_PIN': "Maximum Pin:",
						'Y_ENABLE_PIN': "Enable Pin:", 'Y_INVERT_DIR': "Invert Direction", 'Y_INVERT_MIN': "Invert Minimum",
						'Y_INVERT_MAX': "Invert Maximum", 'Y_INVERT_ENABLE': "Invert Enable",
						
						'Z_STEP_PIN': "Step Pin:", 'Z_DIR_PIN': "Direction Pin:", 'Z_MIN_PIN': "Minimum Pin:", 'Z_MAX_PIN': "Maximum Pin:",
						'Z_ENABLE_PIN': "Enable Pin:", 'Z_INVERT_DIR': "Invert Direction", 'Z_INVERT_MIN': "Invert Minimum",
						'Z_INVERT_MAX': "Invert Maximum", 'Z_INVERT_ENABLE': "Invert Enable",
						
						'E_STEP_PIN': "Step Pin:", 'E_DIR_PIN': "Direction Pin:",	'E_ENABLE_PIN': "Enable Pin:",
						'E_INVERT_DIR': "Invert Direction", 'E_INVERT_ENABLE': "Invert Enable",
						
						'PS_ON_PIN': "PSU On Pin:", 'PS_MOSFET_PIN': "PSU MOSFET Pin:",
						
						'STEPPER_ENABLE_PIN': "Stepper Enable Pin:", 'STEPPER_INVERT_ENABLE': "Stepper Invert Enable",
						
						'SD_CARD_DETECT': "SD Detect Pin:", 'SD_WRITE_PROTECT': "Write Protect Pin:", 'DEBUG_LED_PIN': "Debug LED Pin:" }
...
\\***************************************************************************/\n")
		...
		for k, cttype in self.pinXKeys + self.pinYKeys + self.pinZKeys + self.pinEKeys + self.pinMiscKeys:
			if cttype == 1:
...
class SensorsPage(wx.Panel, Page):
	def __init__(self, parent, nb, idPg):
		...
		self.sensorTypeKeys = [ 'TEMP_MAX6675', 'TEMP_THERMISTOR', 'TEMP_AD595', 'TEMP_PT100', 'TEMP_INTERCOM' ]
		self.sensorType = { 'TEMP_MAX6675': 'TT_MAX6675"', 'TEMP_THERMISTOR': 'TT_THERMISTOR', 'TEMP_AD595': 'TT_AD595',
						'TEMP_PT100': 'TT_PT100', 'TEMP_INTERCOM': 'TT_INTERCOM' }
		self.labels = { 'TEMP_HYSTERESIS': "Temperature hysteresis:", 'TEMP_RESIDENCY_TIME': "Temperature residency time:",			
						'TEMP_EWMA': "Temperature EMWA:", 
						'TEMP_MAX6675': " MAX6675", 'TEMP_THERMISTOR': "Thermistor", 'TEMP_AD595': 'AD595',
...
class SensorList(wx.ListCtrl):
	def __init__(self, parent):
		...
		wx.ListCtrl.__init__(
			self, parent, wx.ID_ANY, size=(495+4, 100),
...
class AddSensorDlg(wx.Dialog):
	def __init__(self, parent, names, sl, arduinoFlag):
		...
		lsz = wx.BoxSizer(wx.HORIZONTAL)
		st = wx.StaticText(self, wx.ID_ANY, "Sensor Name:", size=(80, -1), style=wx.ALIGN_RIGHT)
		lsz.Add(st)
		...
		lsz = wx.BoxSizer(wx.HORIZONTAL)
		st = wx.StaticText(self, wx.ID_ANY, "Additional:", size=(80, -1), style=wx.ALIGN_RIGHT)
		lsz.Add(st)
...
class HeaterList(wx.ListCtrl):
	def __init__(self, parent):
		...
		self.currentItem = None
		wx.ListCtrl.__init__(
			self, parent, wx.ID_ANY, size=(165+4, 100),
			style=wx.LC_REPORT|wx.LC_VIRTUAL|wx.LC_HRULES|wx.LC_VRULES
			)
...
class CommunicationsPage(wx.Panel, Page):
	def __init__(self, parent, nb, idPg):
		...
		self.labels = { 'BAUD': "Baud rate:", 'USB_SERIAL': "USB Serial", 'XONXOFF': "Use XON/XOFF flow control" }
		
		sz = wx.GridBagSizer()
		sz.AddSpacer((20,40), pos=(0,0))

		ch = self.addChoice('BAUD', self.bauds, self.bauds.index(self.defaultBaud), 60, None, self.onChoice)
		sz.Add(ch, pos=(1,1))
...
class MiscellaneousPage(wx.Panel, Page):
	def __init__(self, parent, nb, idPg):
		...
		self.labels = { 'MOTHERBOARD': "Motherboard", 'F_CPU': "CPU Clock Rate:",
					'EECONFIG': "Enable EEPROM Storage", 'DEBUG': "Turn on debugging", 'BANG_BANG': "Enable",
					...
					'TH_COUNT': "Temperature History size:", 'FAST_PWM': "Fast PWM", 'ENDSTOP_STEPS': "Endstop steps:",
		...
		self.references = [ self.defaultRef, 'REFERENCE_AREF', 'REFERENCE_1V1', 'REFERENCE_2V56' ]
		...
		k = "F_CPU"
		ch = self.addChoice(k, self.clocks, self.clocks.index(self.defaultClock), labelWidth, None, self.onChoice)
		sz.Add(ch, pos=(1,3))
		
		k = "REFERENCE"
		ch = self.addChoice(k, self.references, self.references.index(self.defaultRef), labelWidth, None, self.onChoice)
		sz.Add(ch, pos=(2,3))
...
class MyFrame(wx.Frame):
	def __init__(self):
		wx.Frame.__init__(self, None, -1, "Teacup Firmware Configurator", size=(980, 450))
	...
	def onLoadConfig(self, evt):
		...
		dlg = wx.FileDialog(
			self, message="Choose a Config file",
			defaultDir=self.dir, 
			defaultFile="",
			wildcard=wildcard,
...
Please add %s to "supportedCPUs"' % (a, ", ".join(inv), b),
	...
	def onSaveConfig(self, evt):
		...
		fp.write(
"// Generated by Teacup Configurator version %s - %s\n\n" % (VERSION, timestamp))
