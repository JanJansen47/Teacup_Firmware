...
class MyFrame(wx.Frame):
	...
	def __init__(self):
		...
		self.t = 0
		self.seq = 1
		wx.Frame.__init__(self, None, -1, "Teacup Firmware Configurator", size=(450, 600))
		...
		self.tree = CT.CustomTreeCtrl(panel, wx.ID_ANY, wx.DefaultPosition, (100,100), style, agwStyle)
		...
		pins    = self.tree.AppendItem(self.root, "3. Pinouts", data="Pinouts")
		sensors = self.tree.AppendItem(self.root, "4. Temperature Sensors", data="Sensors")
		heaters = self.tree.AppendItem(self.root, "5. Heaters", data="Heaters")
		comm    = self.tree.AppendItem(self.root, "6. Communications", data="Communications")
		misc    = self.tree.AppendItem(self.root, "7. Miscellaneous", data="Miscellaneous")
	...
	def mechSubTree(self, tree, root):
		...
		self.mechCfgKeys = { 'SMX': "STEPS_PER_M_X", 'SMY': "STEPS_PER_M_Y", 'SMZ': "STEPS_PER_M_Z", 'SME' : "STEPS_PER_M_E",
						'MFRX': "MAXIMUM_FEEDRATE_X", 'MFRY': "MAXIMUM_FEEDRATE_Y", 'MFRZ': "MAXIMUM_FEEDRATE_Z", 'MFRE': "MAXIMUM_FEEDRATE_E",
						'MSRX': "SEARCH_FEEDRATE_X", 'MSRY': "SEARCH_FEEDRATE_Y", 'MSRZ': "SEARCH_FEEDRATE_Z",
						'ECX': "ENDSTOP_CLEARANCE_X", 'ECY': "ENDSTOP_CLEARANCE_Y", 'ECZ': "ENDSTOP_CLEARANCE_Z",
		...
		for tag, tbl, cttype in [('SPM', spmKeys, 0), ('MFR', mfrKeys, 0), ('MSR', msrKeys, 0), ('ECL', eclKeys, 0), ('MINMAX', minmaxKeys, 1)]:
			...
			for k in tbl:
				...
				self.tcMech[k] = tc
				self.brMech[k] = tree.AppendItem(st, mechLabels[k], ct_type=0, wnd = tc, data=k)
	...
	def accSubTree(self, tree, root):
		accLabels = { 'ACTYPE': "Acceleration Type:",
						'ACRR': "RepRap", 'ACRP': "Ramping", 'ACTP': "Temporal", 'ACCEL' : "Acceleration", 'LKAH': "Look Ahead",
						'JERK': "Maximum Jerk:",
						'JERKX': "X", 'JERKY': "Y", 'JERKZ': "Z", 'JERKE': "E" }

		self.accCfgKeys = { 'ACRR': "ACCELERATION_REPRAP", 'ACRP': "ACCELERATION_RAMPING", 'ACTP': "ACCELERATION_TEMPORAL",
						'ACCEL': "ACCELERATION", 'LKAH': "LOOKAHEAD",
						'JERKX': "MAX_JERK_X", 'JERKY': "MAX_JERK_Y", 'JERKZ': "MAX_JERK_Z", 'JERKE': "MAX_JERK_E" }
		...
		br = self.brAcc[tag]
		self.brAcc['ACCEL'] = tree.AppendItem(br, accLabels['ACCEL'], ct_type=0, wnd=self.tcAcc['ACCEL'], data='ACCEL')
		self.brAcc['LKAH'] = tree.AppendItem(br, accLabels['LKAH'], ct_type=1, data='LKAH')
		...
		for k in [ 'JERKX', 'JERKY', 'JERKZ', 'JERKE' ]:
			...
			tc.Bind(wx.EVT_CHAR, self.onTextCtrl)
			self.tcAcc[k] = tc
			self.brAcc[k] = tree.AppendItem(st, accLabels[k], ct_type=0, wnd = tc, data=k)
